...
const ManageJobs = () => {
  ...
  const handleToggleStatus = async (jobId, currentStatus) => {
    const isCurrentlyActive = currentStatus === 'active' || currentStatus === 'Active' || currentStatus === true;
      ...
      <div className="space-y-6">
        ...
        {loading ? (
          <div className="py-16 text-center text-slate-400 space-y-3">
            ...
            <p className="text-xs font-semibold">Loading internship postings...</p>
          </div>
        ) : jobs.length === 0 ? (
          <Card variant="glass" className="p-12 text-center text-slate-400 space-y-4">
            <Briefcase className="w-12 h-12 text-slate-600 mx-auto" />
            <h4 className="text-lg font-bold text-white">No Internship Postings Found</h4>
            <p className="text-xs text-slate-400 max-w-sm mx-auto">
              You haven't posted any internship opportunities yet. Create a posting to receive applications from top student candidates.
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {jobs.map((job) => {
              const isActive = job.status === 'active' || job.status === 'Active' || job.isActive === true;
